package net.ctrdn.stuba.pks.netanalyser.am.https;
...
@Analyser(
        name = "HTTPS/SSL/TLS Packet Analyser",
        version = "1.0.0"
)
public class HttpsAnalyserModule implements AnalyserModule<IPv4Frame> {

    public DefaultIPv4ConnectionAnalysisPanel panel = new DefaultIPv4ConnectionAnalysisPanel("HTTPS");
    ...
    public void parse(List<IPv4Frame> frameList, int filteredFrameCount, int totalFrameCount) throws AnalyserModuleException {
        ...
        analysis.addFilter(FilterFactory.tcpPortFilter(FilterPortDirection.SRC_OR_DST_PORT, 443));
        analysis.addFilter(FilterFactory.udpPortFilter(FilterPortDirection.SRC_OR_DST_PORT, 443));
        analysis.analyze();
