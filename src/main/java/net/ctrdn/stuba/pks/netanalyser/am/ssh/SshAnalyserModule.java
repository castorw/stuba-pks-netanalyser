package net.ctrdn.stuba.pks.netanalyser.am.ssh;
...
@Analyser(
        name = "SSH/SFTP Packet Analyser",
        version = "1.0.0"
)
public class SshAnalyserModule implements AnalyserModule<IPv4Frame> {

    public DefaultIPv4ConnectionAnalysisPanel panel = new DefaultIPv4ConnectionAnalysisPanel("SSH");
    ...
    public void parse(List<IPv4Frame> frameList, int filteredFrameCount, int totalFrameCount) throws AnalyserModuleException {
        ...
        analysis.addFilter(FilterFactory.tcpPortFilter(FilterPortDirection.SRC_OR_DST_PORT, 22));
        analysis.addFilter(FilterFactory.udpPortFilter(FilterPortDirection.SRC_OR_DST_PORT, 22));
        analysis.analyze();
